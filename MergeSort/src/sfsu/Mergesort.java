...
public class Mergesort<T extends Comparable> {
    ...
    public static <T extends Comparable> Collection<T> sort(Collection<T> collection) {
        return doSort(new ArrayList<T>(collection));
    }
     ...
     * @return
     */
    private static <T extends Comparable> ArrayList<T> doSort(ArrayList<T> collection) {
        ...
        while (!left.isEmpty() || !right.isEmpty()) {
            ...
            T first = (left.get(0).compareTo(right.get(0)) <= 0)
                    ? left.remove(0) : right.remove(0);
            result.add(first);
        }
        return result;
    }
